...
class regis:
    def __init__(self, root):
        ...
        self.left_frame = Frame(self.root, width=540, height=600, bg="gray",bd=0, highlightthickness=5, highlightbackground="orange")
        self.left_frame.place(x=40,y=0,width=400,height=600)

        # Thêm tiêu đề "THÔNG TIN" và các ô nhập thông tin vào frame trái
        title_label = Label(self.left_frame, text="THÔNG TIN XE", font=("Arial", 20),bg="gray")
        title_label.pack(pady=20)

        Label(self.left_frame, text="Tên chủ:", font=("Arial", 12),bg="gray").pack(pady=10)
        self.ten_chu_entry = Entry(self.left_frame, font=("Arial", 12), width=30)
        self.ten_chu_entry.pack()

        Label(self.left_frame, text="ID:", font=("Arial", 12),bg="gray").pack(pady=10)
        self.id_entry = Entry(self.left_frame, font=("Arial", 12), width=30)
        self.id_entry.pack()

        Label(self.left_frame, text="Số điện thoại:", font=("Arial", 12),bg="gray").pack(pady=10)
        self.sdt_entry = Entry(self.left_frame, font=("Arial", 12), width=30)
        self.sdt_entry.pack()

        Label(self.left_frame, text="CCCD:", font=("Arial", 12),bg="gray").pack(pady=10)
        self.cccd_entry = Entry(self.left_frame, font=("Arial", 12), width=30)
        self.cccd_entry.pack()

        Label(self.left_frame, text="Biển số:", font=("Arial", 12),bg="gray").pack(pady=10)
        ...
        title_label = Label(self.right_frame, text="ĐĂNG KÍ", font=("Arial", 20),bg="gray")
        title_label.pack(pady=20)

        #Tạo frame chứa bảng giá
        price_frame = Frame(self.right_frame, bg="gray", bd=0, highlightthickness=2,
                                highlightbackground="pink")
        price_frame.place(x=360 , y =70 , width=220,height=430)

        title_price_frame = Frame(price_frame,bg="purple", bd=0, highlightthickness=2,
                                highlightbackground="pink")
        title_price_frame.place(x=0,y=0,width=100,height=40)
        title_price = Label(title_price_frame, text="BẢNG GIÁ", font=("Arial", 12),bg="purple")
        title_price.place(x=5 , y = 5 )
        # Giờ
        Label(price_frame, text="Giờ :", font=("Arial", 12),bg="gray").place(x = 25 , y = 80 , width=50 , height=30)
        ...
        # Ngày
        Label(price_frame, text="Ngày :", font=("Arial", 12), bg="gray").place(x=25, y=160, width=50, height=30)
        quantity_entry = Entry(price_frame, font=("Arial", 12), width=30)
        quantity_entry.place(x=85, y=160, width=120, height=30)
        quantity_entry.insert(0,"30000 VND")
        # Tuần
        Label(price_frame, text="Tuần :", font=("Arial", 12), bg="gray").place(x=25, y=240, width=50, height=30)
        quantity_entry = Entry(price_frame, font=("Arial", 12), width=30)
        quantity_entry.place(x=85, y=240, width=120, height=30)
        quantity_entry.insert(0,"500000 VND")
        # Tháng
        Label(price_frame, text="Tháng :", font=("Arial", 12), bg="gray").place(x=25, y=320, width=50, height=30)
        quantity_entry = Entry(price_frame, font=("Arial", 12), width=30)
        quantity_entry.place(x=85, y=320, width=120, height=30)
        quantity_entry.insert(0,"1500000 VND")
        # Tạo frame chứa các radio button
        radio_frame = Frame(self.right_frame, bg="gray", bd=0, highlightthickness=2,
                                highlightbackground="pink")
        ...
        #time_var.set("Giờ")
        hour_radio = Radiobutton(radio_frame, text="Giờ", variable=self.time_var, value="Giờ",bg="gray")
        hour_radio.place(x=80,y=10)
        day_radio = Radiobutton(radio_frame, text="Ngày", variable=self.time_var, value="Ngày",bg="gray")
        day_radio.place(x=80,y=60)
        week_radio = Radiobutton(radio_frame, text="Tuần", variable=self.time_var, value="Tuần",bg="gray")
        week_radio.place(x=80,y=110)
        month_radio = Radiobutton(radio_frame, text="Tháng", variable=self.time_var, value="Tháng",bg="gray")
        month_radio.place(x=80,y=160)

        # Tạo frame chứa so luong va thanh tien
        frame = Frame(self.right_frame, bg="gray", bd=0, highlightthickness=2,
                                highlightbackground="pink")
        frame.place(x=30 , y =300 , width=300,height=200)
        # Thêm ô nhập "số lượng"
        Label(frame, text="Số lượng:", font=("Arial", 12),bg="gray").pack(pady=10)
        self.quantity_entry = Entry(frame, font=("Arial", 12), width=30)
        self.quantity_entry.pack()

        # Thêm ô nhập "Thành tiền"
        Button(frame, text="Thành tiền", font=("Arial", 12), bg="gray", command=self.calculate_total).pack(pady=10)
