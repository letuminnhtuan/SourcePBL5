...
class QL_Camera:
    def __init__(self, master):
        ...
        icon_label2 = Label(
            self.frame_left, image=self.icon_image2, bg="white")
        ...
        icon_label2 = Label(
            self.frame_right, image=self.icon_image1, bg="white")
        icon_label2.place(x=220, y=0)
        # Title
        label_xe_vao = Label(self.frame_left, text="Xe vào", font=(
            "Impact", 22, "italic"), fg="gray", bg="white").place(x=350, y=40, width=90, height=50)
        label_xe_ra = Label(self.frame_right, text="Xe ra", font=(
            "Impact", 22, "italic"), fg="gray", bg="white").place(x=350, y=40, width=90, height=50)

        # Thêm Label để hiển thị hình ảnh từ camera trái
        self.label_camera_left = tk.Label(
            self.frame_left, width=650, height=430, bg="white")
        self.label_camera_left.pack(padx=10, pady=10)
        # Chỉnh vị trí label chứa camera ra giữa
        self.label_camera_left.place(relx=0.5, rely=0.47, anchor=CENTER)
        self.label_camera_left.config(width=650, height=430)

        # Thêm Label để hiển thị hình ảnh từ camera phải
        self.label_camera_right = tk.Label(
            self.frame_right, width=650, height=430, bg="white")
        self.label_camera_right.pack(padx=10, pady=10)
        # Chỉnh vị trí label chứa camera ra giữa
        self.label_camera_right.place(relx=0.5, rely=0.47, anchor=CENTER)
        self.label_camera_right.config(
            width=650, height=430)  # Chỉnh kích thước camera

        # --------- KHỞI TẠO CAMERA TRÁI ------------
        # khởi tạo biến cap_left để lấy camera bên trái
        ...
        # Khởi tạo biến cap_right để lấy camera bên phải
        ...
        Label(self.frame_right, text="Id ", font=("Goudy old style", 14, "bold"), fg="grey", bg="white").place(x=60,
                                                                                                               y=625)
        self.id = Entry(self.frame_right, font=(
            "Goudy old style", 7), bg="#E7E6E6", state="normal")
        ...
        Label(self.frame_right, text="Time out", font=("Goudy old style", 14, "bold"), fg="grey", bg="white").place(x=60,
                                                                                                                    y=675)
        self.timeout = Entry(self.frame_right, font=(
            "Goudy old style", 7), bg="#E7E6E6", state="normal")
        ...
        Label(self.frame_right, text="Biển số", font=("Goudy old style", 14, "bold"), fg="grey", bg="white").place(x=60,
                                                                                                                   y=725)
        self.bienso = Entry(self.frame_right, font=(
            "Goudy old style", 7), bg="#E7E6E6", state="normal")
        ...
        Label(self.frame_left, text="Id ", font=("Goudy old style", 14, "bold"), fg="grey", bg="white").place(x=60,
                                                                                                              y=625)
        self.id = Entry(self.frame_left, font=(
            "Goudy old style", 7), bg="#E7E6E6", state="normal")
        self.id.place(x=270, y=620, width=320, height=35)
        ...
        Label(self.frame_left, text="Time in", font=("Goudy old style", 14, "bold"), fg="grey", bg="white").place(
            x=60,
            y=675)
        self.timein = Entry(self.frame_left, font=(
            "Goudy old style", 7), bg="#E7E6E6", state="normal")
        ...
        Label(self.frame_left, text="Biển số", font=("Goudy old style", 14, "bold"), fg="grey", bg="white").place(x=60,
                                                                                                                  y=725)
        self.bienso = Entry(self.frame_left, font=(
            "Goudy old style", 7), bg="#E7E6E6", state="normal")
        self.bienso.place(x=270, y=720, width=320, height=35)
    ...
    def update_camera_left(self):
        data_thread = threading.Thread(
            target=data_handler, args=(arData, self.cap_left,))
        data_thread.start()
        while True:  # Lặp vô hạn để lấy hình ảnh từ camera
            # Gọi phương thức read của đối tượng cap_left ở trên để lấy hình ảnh từ camera trái
    ...
    def update_camera_right(self):
        while True:  # Lặp vô hạn để lấy hình ảnh từ camera
            ...
            # Gọi phương thức read của đối tượng cap_left ở trên để lấy hình ảnh từ camera phải
